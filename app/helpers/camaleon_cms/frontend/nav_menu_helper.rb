...
module CamaleonCms::Frontend::NavMenuHelper
  ...
  def draw_menu(args = {})
    ...
    if nav_menu.present?
      if args[:show_home_button]
        home_button = if I18n.locale.to_s == 'en'
                        "<li class='home'><a href='/welcome?cama_set_language=#{I18n.locale}'><i lang= '' class='fa fa-home'></i></a></li>"
                      else
                        "<li class='home'><a href='/#{I18n.locale}/welcome?cama_set_language=#{I18n.locale}'><i lang= '' class='fa fa-home'></i></a></li>"
                      end
  ...
  def cama_menu_draw_items(args, nav_menu, level = 0)
    ...
    nav_menu.eager_load(:metas).each do |nav_menu_item|
      _args = args.dup
      nav_menu_item.api_user_role = args[:api_user_role] if args[:api_user_role].present?
      ...
      html += "<#{_args[:item_container]} #{r[:item_container_attrs]} class='#{_args[:item_class]} #{_args[:item_class_parent] if has_children} #{(_args[:item_current]).to_s if _is_current} #{'current-menu-ancestor' if current_children}'>#{_args[:link_before]}
